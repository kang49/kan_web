<template>
    ...
    <div class="w-full min-h-[calc(100dvh)]">
        <div class="w-full h-max pt-[100px] px-[20px]">
            <h1 v-if="page_lang === 'EN'" class="text-center text-black text-[20px] font-bold">Personal Blogs</h1>
            <h1 v-if="page_lang === 'TH'" class="text-center text-black text-[20px] font-bold">บล็อกส่วนตัว</h1>

            <div class="w-full h-max pt-[50px] flex items-center">
                <i class="far fa-tags mr-[5px]"></i>
                <h2 v-if="page_lang === 'EN'" class="text-black text-[18px] font-bold">Tags</h2>
                <h2 v-if="page_lang === 'TH'" class="text-black text-[18px] font-bold">แท็ก</h2>
            </div>
            <div class="w-[80%] h-max pt-[10px] px-[20px]">
                ...
                <NuxtLink to="/tags" class="flex items-center w-max h-max pt-[10px]">
                    <h4 v-if="page_lang === 'EN'" class="text-[14px] font-bold">View all</h4>
                    <h4 v-if="page_lang === 'TH'" class="text-[14px] font-bold">ดูทั้งหมด</h4>
                    <i class="fal fa-long-arrow-right text-[22px] px-[10px]"></i>
                </NuxtLink>
            </div>

            <h2 v-if="page_lang === 'EN'" class="text-black text-[18px] font-bold pt-[50px]">Recents</h2>
            <h2 v-if="page_lang === 'TH'" class="text-black text-[18px] font-bold pt-[50px]">ล่าสุด</h2>
                ...
                <article v-if="content_data" v-for="post in content_data" class="mb-4 h-max">
                    <NuxtLink :to="post._path" class="flex justify-between w-full h-max">
                        ...
                        <NuxtImg 
                            ...
                            :src="post.image"
                            loading="lazy"
                            class="w-[120px] h-[70px] bg-[#9D9D9D9D] object-scale-down rounded-[10px]"
                            alt="Blog image"
                        />
                        <div 
                            v-else 
                            class="w-[120px] h-[70px] bg-[#e0e0e0] rounded-[10px]"
                        >
